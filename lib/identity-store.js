...
function buildWhereClause(where, params = []) {
  ...
  if (where.fieldFilter) {
    ...
    console.log(`[buildWhereClause] fieldFilter: fieldName="${fieldName}", op="${op}", value=${JSON.stringify(value)}`);
  ...
  if (where.compositeFilter) {
    ...
    console.log(`[buildWhereClause] compositeFilter: op="${op}", filters count=${filters.length}`);
    
    if (!filters.length) return { sql: "", params };

    const parts = filters.map((child, idx) => {
      const parsed = buildWhereClause(child, params);
      console.log(`[buildWhereClause] filter[${idx}] sql="${parsed.sql}"`);
    ...
    console.log(`[buildWhereClause] final SQL: "${finalSql}"`);
